...
<head>
    <title>HITUNG KELILING</title>
</head>
<body>
    <h1 style="text-align: center; padding: 50px; font-size: 50px;">HITUNG KELILING BELAH KETUPAT</h1>
<div class="container">
    <h2>Masukkan Nilai :</h2>
    <form action="hitungan_keliling_ketupat.php" method="POST" enctype="multipart/form-data">
        <div>
            <p>Sisi :</p>
            <hr style="margin-top: -10px; margin-right: 5px;">
            <input type="text" name="sisi" placeholder="Masukkan nilai" required style="width: 250px; height: 50px; font-size: 20px; text-align: center;border-radius: 20px; font-weight: bold;">
        </div>
        <div>
    ...
    error_reporting(0);
        if(isset($_POST['submit'])){
            ...
            $keliling_dketupat = 4 * $s;
            echo "Hasil hitung keliling belah ketupat adalah sebagai berikut :<br />";
            echo "Diketahui :<br />";
            echo "Sisi = $s<br />";
            echo "<br>";
            echo "Maka luas belah ketupat sama dengan 4 x $s = $keliling_dketupat cm";
        }
